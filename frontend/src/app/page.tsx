...
function HomeContent() {
  ...
  const handlePlayVsPlayer = useCallback(async () => {
    ...
    // In a real implementation, this would call a joinQueue action
    // For now, we simulate by creating a game and waiting for an opponent
    showNotification("info", "Searching for opponent...");

    // Simulate matchmaking - in production this would be handled by the backend
    // For demo purposes, we create a pending game after a short delay
    setTimeout(async () => {
      try {
        const success = await createGameAction(false, selectedTimeControl, undefined, true);
        if (success) {
          const { games: updatedGames } = useGameStore.getState();
          const newGame = updatedGames[updatedGames.length - 1];
          if (newGame) {
            selectGame(newGame.id);
            // Stay in queue view until opponent joins
          }
        }
      } catch (error) {
        console.error("Failed to create game for matchmaking:", error);
      }
    }, 1000);
  }, [ready, selectedTimeControl, createGameAction, selectGame, showNotification]);
