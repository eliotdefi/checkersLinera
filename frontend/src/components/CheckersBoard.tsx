...
export default function CheckersBoard({
  ...
  // Track previous boardState for debugging
  const prevBoardState = useRef(boardState);
  useEffect(() => {
    if (prevBoardState.current !== boardState) {
      console.log("[CheckersBoard] boardState CHANGED:", {
        prev: prevBoardState.current.substring(0, 30) + "...",
        curr: boardState.substring(0, 30) + "...",
      });
      prevBoardState.current = boardState;
    }
  }, [boardState]);

  // Reset selection state when board or game changes
  useEffect(() => {
    ...
    // Don't clear premove here - it's managed by parent
  }, [boardState, disabled]);

  // DEBUG: Log all props on every render
  console.log("[CheckersBoard] RENDER with props:", {
    disabled,
    isMyTurn,
    currentTurn,
    playerColor,
    boardStateSnippet: boardState?.substring(0, 20),
  });

  // Parse board state to 2D array
  const board = useMemo(() => {
    console.log("[CheckersBoard] Parsing boardState:", boardState.substring(0, 30) + "...");
  ...
  const handleSquareClick = useCallback(
    (row: number, col: number) => {
      console.log("[CheckersBoard] Click:", { row, col, disabled, isMyTurn, currentTurn, playerColor });

      // If game is disabled (ended), don't allow any clicks
      if (disabled) {
        console.log("[CheckersBoard] Click BLOCKED - disabled");
        return;
      }
