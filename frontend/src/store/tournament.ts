...
const CREATE_TOURNAMENT_MUTATION = `
  mutation CreateTournament($name: String!, $timeControl: TimeControl!, $maxPlayers: Int!, $isPublic: Boolean!, $scheduledStart: Int) {
    createTournament(name: $name, timeControl: $timeControl, maxPlayers: $maxPlayers, isPublic: $isPublic, scheduledStart: $scheduledStart)
  }
`
...
const JOIN_TOURNAMENT_BY_CODE_MUTATION = `
  mutation JoinTournamentByCode($inviteCode: String!) {
    joinTournamentByCode(inviteCode: $inviteCode)
  }
`

const LEAVE_TOURNAMENT_MUTATION = `
  mutation LeaveTournament($tournamentId: String!) {
...
const START_TOURNAMENT_MATCH_MUTATION = `
  mutation StartTournamentMatch($tournamentId: String!, $matchId: String!) {
    startTournamentMatch(tournamentId: $tournamentId, matchId: $matchId)
  }
`
...
export const useTournamentStore = create<TournamentStore>((set, get) => ({
  ...
  createTournament: async (name: string, timeControl: TimeControl, maxPlayers: number, isPublic: boolean, scheduledStart?: number) => {
    ...
    try {
      ...
      let mutationQuery = `mutation { createTournament(name: "${escapedName}", timeControl: ${timeControl}, maxPlayers: ${maxPlayers}, isPublic: ${isPublic}`
  ...
  joinTournamentByCode: async (code: string) => {
    ...
    try {
      const data = await requestAsync<{ joinTournamentByCode: string }>({
        ...
        variables: { inviteCode: code.toUpperCase() },
      })
      set({ isLoading: false })
      // Refresh tournaments
      get().fetchTournaments()
