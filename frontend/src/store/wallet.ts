...
function getStoredChainId(): string | null {
  ...
  if (stored && stored !== DEFAULT_CHAIN_ID && DEFAULT_CHAIN_ID) {
    console.warn('[Wallet] Clearing stale chain ID:', stored)
...
export const useWalletStore = create<WalletStore>((set, get) => ({
  ...
  init: () => {
    ...
    console.log('[WalletStore] Player ID:', playerId)
    console.log('[WalletStore] Default chain:', defaultChain)
    ...
    if (defaultChain) {
      ...
      console.log(`[WalletStore] Auto-connected as: ${playerId}`)
      console.log(`[WalletStore] To test as different player, open an Incognito window or different browser`)
    } else {
      set({
        chainId: null,
        playerId,
  ...
  connect: (chainId: string, port?: string) => {
    ...
    try {
      ...
      console.log('[WalletStore] Connected to:', chainId.slice(0, 16) + '...')
    } catch (e) {
      set({
        isConnecting: false,
  ...
  disconnect: () => {
    ...
    console.log('[WalletStore] Disconnected')
  },
