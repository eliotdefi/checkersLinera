...
const JOIN_QUEUE_MUTATION = `mutation JoinQueue($timeControl: TimeControl!) { joinQueue(timeControl: $timeControl) }`
const LEAVE_QUEUE_MUTATION = `mutation LeaveQueue { leaveQueue }`
...
export const useGameStore = create<GameStore>((set, get) => ({
      ...
      if (selectedGameId) {
        ...
        if (updated) {
          ...
          if (!currentSelectedGame || updated.moveCount >= currentSelectedGame.moveCount) {
            ...
            console.log('[GameStore] Skipping stale update from fetchGames, current moveCount:', currentSelectedGame.moveCount, 'fetched:', updated.moveCount)
          }
        }
      }
    } catch (e) {
  ...
  fetchGame: async (id: string) => {
    ...
    try {
      ...
      if (data?.game) {
        ...
        console.log('[GameStore] fetchGame received:', {
          id: data.game.id,
  ...
  fetchMyStats: async () => {
    ...
    if (!playerId) {
      console.log('[GameStore] No playerId, skipping fetchMyStats')
      return
    }
    try {
  ...
  createGame: async (
    ...
    try {
      ...
      console.log('[GameStore] createGame raw result:', JSON.stringify(result))
      ...
      console.log('[GameStore] createGame - newest pending game:', newest?.id)
  ...
  makeMove: async (from: MoveInfo, _to: MoveInfo) => {
    ...
    if (get().isMoving) {
      console.log('[GameStore] Move already in progress, ignoring')
      return false
    }
    ...
    try {
      console.log('[GameStore] Sending move:', from, 'playerId:', playerId)
      ...
      console.log('[GameStore] Move sent successfully, waiting for blockchain...')
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (get().selectedGameId !== selectedGameId) {
          console.log('[GameStore] Game changed during move, aborting retry')
          break
        }
        ...
        // Debug logging for moveCount comparison
        console.log('[GameStore] Retry', i + 1, '- oldMoveCount:', oldMoveCount, 'newMoveCount:', updatedGame?.moveCount, 'type:', typeof updatedGame?.moveCount)

        // Verify we're still on the same game
        if (updatedGame?.id !== selectedGameId) {
          console.log('[GameStore] Game switched, stopping retry')
          break
        }

        if (updatedGame && updatedGame.moveCount > oldMoveCount) {
          console.log('[GameStore] Game state updated successfully, moveCount:', updatedGame.moveCount)
          fetchSuccess = true
          break
        }

        console.log(`[GameStore] Retry ${i + 1}/3 - waiting for state update...`)
        await new Promise(resolve => setTimeout(resolve, 500))
      }
      ...
      if (!fetchSuccess) {
        console.warn('[GameStore] Game state may not have updated properly after 3 retries')
        // Still return true as the mutation was sent - the UI will update on next poll
      }
  ...
  rollbackMove: () => {
    ...
    if (previousState) {
      ...
      console.log('[GameStore] Move rolled back')
    }
  },
