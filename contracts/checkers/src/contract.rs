...
use checkers_abi::{
    ...
    GameStatus, MatchStatus, Message, Operation, OperationResult, Piece, PlayerType, TimeControl,
    Tournament, TournamentMatch, TournamentStatus, Turn,
...
impl Contract for CheckersContract {
    ...
    async fn execute_operation(&mut self, operation: Self::Operation) -> Self::Response {
        match operation {
            ...
            Operation::JoinQueue { time_control } => self.join_queue(time_control).await,
            Operation::LeaveQueue => self.leave_queue().await,
            ...
            Operation::CreateTournament { name, time_control, max_players, is_public, scheduled_start } => {
                self.create_tournament(name, time_control, max_players, is_public, scheduled_start).await
            }
            Operation::JoinTournament { tournament_id } => {
                self.join_tournament(tournament_id).await
            }
            Operation::JoinTournamentByCode { invite_code } => {
                self.join_tournament_by_code(invite_code).await
            }
            Operation::LeaveTournament { tournament_id } => {
                self.leave_tournament(tournament_id).await
            }
            Operation::StartTournament { tournament_id } => {
                self.start_tournament(tournament_id).await
            }
            Operation::StartTournamentMatch { tournament_id, match_id } => {
                self.start_tournament_match(tournament_id, match_id).await
            }
        }
    }
...
impl CheckersContract {
    ...
    async fn join_queue(&mut self, time_control: TimeControl) -> OperationResult {
        let chain_id = self.runtime.chain_id().to_string();
        let timestamp = self.runtime.system_time().micros();
        let timestamp_ms = timestamp / 1000;

        match self.state.join_queue(&chain_id, time_control, timestamp).await {
            Ok(Some(opponent_chain_id)) => {
                ...
                game.black_player = Some(chain_id.clone());
                game.black_player_type = PlayerType::Human;
                game.status = GameStatus::Active;
                game.created_at = timestamp;
                game.updated_at = timestamp;
    ...
    async fn leave_queue(&mut self) -> OperationResult {
        let chain_id = self.runtime.chain_id().to_string();

        match self.state.leave_queue(&chain_id).await {
    ...
    async fn create_tournament(
        ...
        if ![4, 8, 16, 32].contains(&max_players) {
            return OperationResult::Error {
                message: "Max players must be 4, 8, 16, or 32".to_string(),
            };
        }

        let creator = self.runtime.chain_id().to_string();
        ...
        let invite_code = if !is_public {
            Some(self.generate_invite_code(timestamp))
        } else {
            None
        };
    ...
    fn generate_invite_code(&self, timestamp: u64) -> String {
        ...
        for _ in 0..6 {
            let idx = (seed % 32) as usize;
            code.push(CHARS[idx] as char);
            seed = seed / 32 + seed % 17 + 7; // Simple pseudo-random mixing
        }

        code
    }

    async fn join_tournament(&mut self, tournament_id: String) -> OperationResult {
        let player = self.runtime.chain_id().to_string();
    ...
    async fn join_tournament_by_code(&mut self, invite_code: String) -> OperationResult {
        let player = self.runtime.chain_id().to_string();
    ...
    async fn leave_tournament(&mut self, tournament_id: String) -> OperationResult {
        let player = self.runtime.chain_id().to_string();
    ...
    async fn start_tournament(&mut self, tournament_id: String) -> OperationResult {
        let player = self.runtime.chain_id().to_string();
        ...
        if tournament.registered_players.len() < 2 {
            return OperationResult::Error { message: "Need at least 2 players".to_string() };
        }
    ...
    fn generate_bracket(&self, tournament: &mut Tournament) {
        ...
        let bracket_size = tournament.max_players as usize;

        // Get seed order for bracket
        let seed_order = self.get_seed_order(bracket_size);

        // Create Round 1 matches
        let matches_in_round1 = bracket_size / 2;
        for i in 0..matches_in_round1 {
            let seed1_idx = seed_order[i * 2];
            let seed2_idx = seed_order[i * 2 + 1];

            let player1 = tournament.registered_players.get(seed1_idx).cloned();
            let player2 = tournament.registered_players.get(seed2_idx).cloned();

            let status = match (&player1, &player2) {
                (Some(_), None) => MatchStatus::Bye,
                (None, Some(_)) => MatchStatus::Bye,
                (Some(_), Some(_)) => MatchStatus::Ready,
        ...
        for round in 2..=tournament.total_rounds {
    ...
    async fn start_tournament_match(
        ...
        // Create the game
        let game_id = self.state.generate_game_id().await;
        let player1 = tournament_match.player1.clone().unwrap();
        let player2 = tournament_match.player2.clone().unwrap();
    ...
    async fn handle_tournament_game_finished(&mut self, game: &CheckersGame) {
        ...
        let winner = match game.result {
            ...
            _ => return, // Draw or no result - shouldn't happen in tournament
        };
        ...
        self.advance_winner(&mut tournament, &match_id, &winner_id);

        // Check if current round is complete
        let current_round = tournament.current_round;
        let round_complete = tournament.matches.iter()
            .filter(|m| m.round == current_round)
            .all(|m| m.status == MatchStatus::Finished || m.status == MatchStatus::Bye);
